...
// ─── Side nav (passed as sideNav slot to PageLayout) ─────────────────────────
function SideNav({ activePage, setActivePage, themeKey, setThemeKey }) {
  return (
    <>
      {/* Logo */}
      <Stack direction="row" alignItems="center" spacing={1.5} sx={{ px: 2, py: 2, flexShrink: 0 }}>
      ...
      <Typography
        variant="caption"
        sx={{ px: 2, pt: 2, pb: 0.5, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600, display: 'block' }}
      >
        Templates
      </Typography>
      ...
      {/* Theme toggle */}
      <Box sx={{ px: 1.5, pb: 2, pt: 1.5, flexShrink: 0 }}>
        <Typography
          variant="caption"
        ...
        <ToggleButtonGroup
          value={themeKey}
          exclusive
          onChange={(_, val) => val && setThemeKey(val)}
          size="small"
